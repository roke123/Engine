...
//创建一个矩形
bool DrawTool::CreateRectangle2D( const Vertex2D_F& p0, const Vertex2D_F& p1, Rectangle2DType ** pRectangle ) const
{
	*pRectangle = new Rectangle2DType;
	...
	(*pRectangle)->worldVertexCenter.x = fabs( p0.x - p1.x ) / 2;
	(*pRectangle)->worldVertexCenter.y = fabs( p0.y - p1.y ) / 2;

	return true;
}

bool DrawTool::CreateRectangle2D( const Vertex2D_F& p0, int width, int height, Rectangle2DType ** pRectangle ) const
{
	(*pRectangle) = new Rectangle2DType;
	if( pRectangle == NULL )
	{
...
bool DrawTool::CreatePolygon2D( Vertex2D_F * pVertexList, int numVertex, Polygon2DType ** pPolygon ) const
{
	*pPolygon = new Polygon2DType;
	if( pPolygon == NULL )
	{
		return false;
	}
